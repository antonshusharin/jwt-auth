...
func TestGetAndRefreshToken(t *testing.T) {
	...
	require.Equal(http.StatusOK, status, "Could not retrieve access token")
	var tokenPair TokenPair
	err := json.Unmarshal(respBody, &tokenPair)
	require.NoError(err, "Could not parse token pair as JSON")
	body2, _ := json.Marshal(tokenPair)
	req = httptest.NewRequest("POST", "/refresh-token", bytes.NewReader(body2))
	status, respBody = makeTestRequest(req)
	require.Equal(http.StatusOK, status, "Could not refresh with provided token pair")
	err = json.Unmarshal(respBody, &tokenPair)
	require.NoError(err, "Could not decode refreshed token pair")
	req = httptest.NewRequest("POST", "/refresh-token", bytes.NewReader(body2))
	status, _ = makeTestRequest(req)
	require.Equal(http.StatusForbidden, status, "Was able to refresh the same token pair twice")
}
...
func TestJWTSecurity(t *testing.T) {
	...
	require.NoError(err, "Could not connect to database")
	defer conn.Release()
	pair1, err := app.generateTokenPair(conn, MOCK_USERS["Ivan"].guid, "192.0.2.1")
	require.NoError(err, "Could not create token pair 1")
	pair2, err := app.generateTokenPair(conn, MOCK_USERS["Maria"].guid, "192.0.2.1")
	require.NoError(err, "Could not create token pair 2")
	...
	status, _ = makeTestRequest(req)
	require.Equal(http.StatusOK, status, "Could not refresh pair 2, which is valid")
	mailer := app.mailer.(*TestingMailer)
	mail := mailer.CheckEmail()
	require.NotNil(t, mailer, "Warning email was not sent")
	require.Equal([]string{MOCK_USERS["Maria"].email}, mail.To, "Warning email was sent to the wrong user")
}
